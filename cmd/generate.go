...
func createRoute(name string) {
	...
	f := NewFile("route")
	f.ImportAlias("grpc-kit-service/controller/"+name, name+"Controller")
	f.Func().Id("(t *T)").Id(strings.Title(name) + "Routes").Params().Block(
		Id("t.router.GET").Params(Id("\"/"+name+"/ping\""), Qual("grpc-kit-service/controller/"+name, "Ping")),
	)
	buf := &bytes.Buffer{}
	if err := f.Render(buf); err != nil {
		panic(err)
...
func createConsumer(name string) {
	...
	f.Func().Params(Id("t *T")).Id(strings.Title(name)+"Consumer").Params().Block(
		Id("msgs, _ :=").Qual("grpc-kit-service/provider/rabbitmq", "Consume").Call(Id("\"ginTestExchange\""), Id("\"ginTestQueue\"")),
